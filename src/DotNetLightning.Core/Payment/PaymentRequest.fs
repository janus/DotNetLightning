...
module private Helpers =
    ...
    let tryGetP2WPKHAddressEncoder(network: Network) =
        let maybeEncoder =
            network.GetBech32Encoder(Bech32Type.WITNESS_PUBKEY_ADDRESS, false)

        if isNull maybeEncoder then
            Error("Failed to get p2wpkh encoder")
        else
            Ok maybeEncoder

    let tryGetP2WSHAddressEncoder(network: Network) =
        let maybeEncoder =
            network.GetBech32Encoder(Bech32Type.WITNESS_SCRIPT_ADDRESS, false)

        if isNull maybeEncoder then
            Error("Failed to get p2wsh encoder")
        else
            Ok maybeEncoder
    ...
    let checkAndGetPrefixFromHrp(hrp: string) =
        let maybePrefix =
            sortedPrefixValues
            |> Seq.filter(fun p -> hrp.StartsWith(p))
            |> Seq.tryHead
        ...
        | None ->
            Error(
                sprintf
                    "Unknown prefix type %s! hrp must be either of %A"
                    hrp
                    sortedPrefixValues
            )
    ...
    let uint64ToBase32(num: uint64) : array<byte> =
        if num = 0UL then
            [||]
        else
...
type IMessageSigner =
    ...
    abstract member SignMessage: uint256 -> array<byte>
...
type FallbackAddress =
    ...
    static member FromBase58Address(addr: string) =
        result {
            ...
            match prefix.[0] with
            | Helpers.PREFIX_ADDRESS_PUBKEYHASH
            | Helpers.PREFIX_ADDRESS_PUBKEYHASH_TESTNET ->
                return
                    {
                        Version = 17uy
                        Data = addressHash
                    }
            | Helpers.PREFIX_ADDRESS_SCRIPTHASH
            | Helpers.PREFIX_ADDRESS_SCRIPTHASH_TESTNET ->
                return
                    {
                        Version = 18uy
                        Data = addressHash
                    }
            | x -> return! Error(sprintf "Unknown address prefix %i" x)
        }
    ...
    static member FromAddress(s: BitcoinAddress) =
        match s with
        | :? BitcoinWitScriptAddress as p2wsh ->
            FallbackAddress.FromBech32Address(p2wsh.ToString(), s.Network)
        | :? BitcoinWitPubKeyAddress as p2wpkh ->
            FallbackAddress.FromBech32Address(p2wpkh.ToString(), s.Network)
        | :? BitcoinPubKeyAddress as p2pkh ->
            FallbackAddress.FromBase58Address(p2pkh.ToString())
        | :? BitcoinScriptAddress as p2pkh ->
            FallbackAddress.FromBase58Address(p2pkh.ToString())
        | _ -> Error(sprintf "Unknown type of address: %s" (s.ToString()))

    static member TryCreate(version, data: array<byte>) =
        match version with
        // p2pkh
        ...
        | v ->
            sprintf
                "Unknown combination of bitcoin address version and length! version %i. length: %i"
                v
                data.Length
            |> Error
    ...
    member this.ToAddress(prefix: string) =
        ...
        | 17uy when prefix = "lnbc" ->
            let data =
                Array.concat(
                    seq {
                        yield [| Helpers.PREFIX_ADDRESS_PUBKEYHASH |]
                        yield this.Data
                    }
                )
        ...
        | 18uy when prefix = "lnbc" ->
            let data =
                Array.concat(
                    seq {
                        yield [| Helpers.PREFIX_ADDRESS_SCRIPTHASH |]
                        yield this.Data
                    }
                )
        ...
        | v when prefix = "lnbcrt" ->
            ...
            encoder.Encode(v, this.Data)
        | v -> failwithf "Unreachable! Unexpected version %A" v
...
type ExtraHop =
    ...
    static member TryCreate
        ...
        if cltvExpiryDelta.Value = 0us then
            Error("CLTVExpiryDelta in ExtraHop should not be 0")
        else
            Ok
                {
                    NodeId = nodeId
...
type TaggedField =
    ...
    | RoutingInfoTaggedField of list<ExtraHop>
    ...
    member private this.WriteField(writer: BinaryWriter, data: array<byte>) =
        let mutable dataLengthInBase32 =
            Helpers.uint64ToBase32(data.Length |> uint64)
        ...
        if (dataLengthInBase32.Length <> 2) then
            ...
            <| FormatException(
                sprintf
                    "data length too-big to fit in 10 bits. It was %i"
                    dataLengthInBase32.Length
            )
    ...
    member internal this.WriteTo(writer: BinaryWriter, timestamp) =
        ...
        | ExpiryTaggedField x ->
            let dBase32 =
                ((x.ToUnixTimeSeconds() |> uint64) - timestamp)
        ...
        | RoutingInfoTaggedField r ->
            ...
            for hopInfo in r do
                ...
                Array.blit
                    (hopInfo.ShortChannelId.ToBytes())
                    0
                    hopInfoBase256
                    33
                    8
                ...
                Array.blit
                    ((hopInfo.FeeBase.MilliSatoshi |> uint32)
                        .GetBytesBigEndian())
                    0
                    hopInfoBase256
                    41
                ...
                Array.blit
                    ((hopInfo.FeeProportionalMillionths |> uint32)
                        .GetBytesBigEndian())
                    0
                    hopInfoBase256
                    45
                ...
                Array.blit
                    (hopInfo.CLTVExpiryDelta.Value.GetBytesBigEndian())
                    0
                    hopInfoBase256
                    49
                    2
            ...
            let routeInfoBase32 =
                routeInfoBase256 |> Array.concat |> Helpers.convert8BitsTo5
...
type TaggedFields =
    ...
    member this.FallbackAddresses =
        ...
        |> List.choose(
            function
            | FallbackAddressTaggedField a -> Some(a)
            | _ -> None
        )
    ...
    member this.FeatureBits =
        ...
        |> Seq.choose(
            function
            | FeaturesTaggedField fb -> Some fb
            | _ -> None
        )
        |> Seq.tryExactlyOne
    ...
    member this.PaymentSecret =
        ...
        |> Seq.choose(
            function
            | PaymentSecretTaggedField ps -> Some ps
            | _ -> None
        )
        |> Seq.tryExactlyOne
    ...
    member this.CheckSanity() =
        let pHashes =
            ...
            |> List.choose(
                function
                | PaymentHashTaggedField x -> Some x
                | _ -> None
            )

        if (pHashes.Length > 1) then
            "duplicate 'p' field" |> Error
        else if (pHashes.Length < 1) then
            "no payment hash" |> Error
        else
            let secrets =
                this.Fields
                |> List.choose(
                    function
                    | PaymentSecretTaggedField x -> Some(x)
                    | _ -> None
                )

            if (secrets.Length > 1) then
                "duplicate 's' field" |> Error
            else if secrets.Length = 1 && this.FeatureBits.IsNone then
                sprintf
                    "secret (%A) is set but there were no feature bits"
                    secrets.[0]
                |> Error
            elif secrets.Length = 1
                 && not
                    <| this.FeatureBits.Value.HasFeature Feature.PaymentSecret then
                let fb = this.FeatureBits.Value

                sprintf
                    "secret (%A) is set but feature bit (%s) is not set (%A)"
                    (secrets.[0])
                    (fb.ToString())
                    (fb)
                |> Error
            else if
                secrets.Length = 0 && this.FeatureBits.IsSome
                && this.FeatureBits.Value.HasFeature
                    (
            ...
            then
                Error
                    "feature bit for payment secret is set but payment secret is not set"
            else
                let descriptions =
                    this.Fields
                    |> List.choose(
                        function
                        | DescriptionTaggedField d -> Some d
                        | _ -> None
                    )

                if (descriptions.Length > 1) then
                    Error("duplicate 'd' field")
                else
                    let dHashes =
                        this.Fields
                        |> List.choose(
                            function
                            | DescriptionHashTaggedField x -> Some x
                            | _ -> None
                        )

                    if (dHashes.Length > 1) then
                        Error("duplicate 'h' field")
                    else if (descriptions.Length = 1 && dHashes.Length = 1) then
                        Error("both 'h' and 'd' field exists")
                    else if (descriptions.Length <> 1 && dHashes.Length <> 1) then
                        Error(
                            "must have either description hash or description"
                        )
                    else if (descriptions.Length = 1
                             && descriptions.[0] |> String.IsNullOrEmpty) then
                        Error("Description should not be an empty string")
                    else if (this.Fields
                             |> List.filter(
                                 function
                                 | TaggedField.RoutingInfoTaggedField _ -> true
                                 | _ -> false
                             )
                             |> List.length > 2) then
                        Error(
                            "Extra Routing hint information can not be longer than 2."
                        )
                    else
                        let features =
                            this.Fields
                        ...
                        | Some x when
                            x.BitArray.Length = 0
                            || x.ToByteArray() |> Seq.forall((=) 0uy)
                            ->
                            Error(
                                "Empty Features are not allowed for PaymentRequest"
                            )
                        | _ ->
                            let maybeMinFinalExp =
                                ...
                                |> List.choose(
                                    function
                                    | MinFinalCltvExpiryTaggedField x -> Some x
                                    | _ -> None
                                )
                                |> List.tryExactlyOne
...
type private Bolt11Data =
    {
        ...
        // byte is recovery id
        Signature: option<(LNECDSASignature * byte)>
    }

    static member FromBytes(b: array<byte>) : Result<Bolt11Data, _> =
        result {
            ...
            if (reader.Position < 0) then
                return!
                    sprintf
                        "Invalid BOLT11: Invalid size. reader.Position was (%i)"
                        reader.Position
                    |> Error
            else if (not <| reader.CanConsume(65)) then
                return!
                    "Invalid BOLT11: Invalid size. could not consume 65"
                    |> Error
            else
                let rs = reader.ReadBytes(65)

                let signature =
                    LNECDSASignature.FromBytesCompact(rs.[0 .. rs.Length - 2])
                ...
                let checkSize (r: BitReader) c =
                    if (not <| r.CanConsume(c)) then
                        Error(
                            sprintf
                                "Invalid BOLT11: Invalid size. could not consume %i"
                                c
                        )
                    else
                        Ok()
                            ...
                            | 16UL -> // payment secret
                                if (size <> 52 * 5) then
                                    ...
                                    let ps =
                                        ...
                                        |> fun x ->
                                            uint256(x, false)
                                            |> PaymentSecretTaggedField

                                    return!
                                        loop
                                            r
                                            { acc with
                                                Fields = ps :: acc.Fields
                                            }
                                            afterReadPosition
                            | 6UL -> // expiry
                                let expiryDate =
                                ...
                                try
                                    ...
                                    return!
                                        loop
                                            ...
                                            ({ acc with
                                                 Fields =
                                                     shortDesc :: acc.Fields
                                             })
                                            afterReadPosition
                                with
                                | _ -> return! loop r acc afterReadPosition
                            ...
                            | 23UL -> // description hash
                                if (size <> 52 * 5) then
                                    ...
                                    let dHash =
                                        ...
                                        |> fun x ->
                                            uint256(x, false)
                                            |> DescriptionHashTaggedField

                                    return!
                                        loop
                                            r
                                            ({ acc with
                                                 Fields = dHash :: acc.Fields
                                             })
                                            afterReadPosition
                            | 3UL -> // routing info
                                if (size < ExtraHop.Size) then
                                    return!
                                        sprintf
                                            "Unexpected length for routing info (%i)"
                                            size
                                        |> Error
                                else
                            ...
                            | 5UL -> // feature bits
                                ...
                                let! fb =
                                    ...
                                    |> Result.mapError(fun x ->
                                        "Invalid BOLT11! Feature field is bogus "
                                        + x.ToString()
                                    )
                ...
                let! taggedField =
                    loop reader (TaggedFields.Zero) reader.Position
    ...
    member this.ToBytesBase32() =
        ...
        let timestamp = Utils.DateTimeToUnixTime(this.Timestamp) |> uint64
        let timestampBase32 = timestamp |> Helpers.uint64ToBase32

        Debug.Assert(
            timestampBase32.Length <= (35 / 5),
            sprintf "Timestamp too big: %i" timestampBase32.Length
        )

        let padding: array<byte> = Array.zeroCreate(7 - timestampBase32.Length)
        writer.Write(padding)
        writer.Write(timestampBase32)

        this.TaggedFields.Fields
        |> List.rev
        |> List.iter(fun f -> f.WriteTo(writer, timestamp))

        this.Signature
        |> Option.iter(fun (s, recv) ->
            let sigBase32 =
                Array.concat [ s.ToBytesCompact(); [| recv |] ]
                |> Helpers.convert8BitsTo5
...
type PaymentRequest =
    ...
    member this.PaymentHash =
        ...
        |> Seq.choose(
            function
            | TaggedField.PaymentHashTaggedField p -> Some p
            | _ -> None
        )
    ...
    member this.PaymentSecret =
        ...
        |> Seq.choose(
            function
            | TaggedField.PaymentSecretTaggedField ps -> Some ps
            | _ -> None
        )
        |> Seq.tryExactlyOne
    ...
    member this.FallbackAddresses =
        ...
        |> List.choose(
            function
            | FallbackAddressTaggedField f -> Some f
            | _ -> None
        )
    ...
    member this.RoutingInfo =
        ...
        |> List.choose(
            function
            | RoutingInfoTaggedField r -> Some r
            | _ -> None
        )
    ...
    member this.Expiry =
        ...
        |> Seq.choose(
            function
            | ExpiryTaggedField e -> Some(e)
            | _ -> None
        )
        |> Seq.tryExactlyOne
        |> Option.defaultValue(
            this.Timestamp + PaymentConstants.DEFAULT_EXPIRY_SECONDS
        )
    ...
    member this.MinFinalCLTVExpiryDelta =
        ...
        |> Seq.choose(
            function
            | MinFinalCltvExpiryTaggedField cltvE -> Some(cltvE)
            | _ -> None
        )
        |> Seq.tryExactlyOne
    ...
    member this.Features =
        ...
        |> Seq.choose(
            function
            | FeaturesTaggedField f -> Some f
            | _ -> None
        )
        |> Seq.tryExactlyOne

    member this.IsExpired = this.Expiry <= DateTimeOffset.UtcNow

    member this.HumanReadablePart =
        (sprintf "%s%s" (this.Prefix) (Amount.encode(this.Amount)))

    member private this.Hash =
        let hrp = this.HumanReadablePart |> Helpers.utf8.GetBytes

        let data =
            {
                Bolt11Data.Timestamp = this.Timestamp
    ...
    member this.Sign(privKey: Key, ?forceLowR: bool) =
        ...
        let sig64 =
            ecdsaSig |> fun s -> LNECDSASignature.FromBytesCompact(s, true)

        { this with
            Signature = (sig64, recvId) |> Some
        }
    ...
    member private this.ToString(signature65bytes: array<byte>) =
        let hrp = this.HumanReadablePart
        ...
        let data =
            ...
            let signature =
                signature65bytes
                |> fun s -> LNECDSASignature.FromBytesCompact(s, true)
    ...
    static member Parse(str: string) : Result<PaymentRequest, string> =
        result {
            ...
            let maybeAmount =
                Amount.decode(hrp.Substring(prefix.Length))
                |> function
                    | Ok s -> Some s
                    | Error _ -> None
            ...
            let nodeId =
                ...
                | None ->
                    let msg =
                        ...
                        let reader =
                            BitReader(bitArray, bitArray.Count - 520 - 30)

                        let remainder = ref 0

                        let mutable byteCount =
                            Math.DivRem(reader.Count, 8, remainder)

                        if (!remainder <> 0) then
                            byteCount <- byteCount + 1

                        seq {
                            yield (hrp |> Helpers.utf8.GetBytes)
                            yield (reader.ReadBytes(byteCount))
                        }
                        |> Array.concat
                    ...
                    Array.blit
                        (sigCompact.ToBytesCompact())
                        0
                        signatureInNBitcoinFormat
                        1
                        64
                    ...
                    PubKey
                        .RecoverCompact(r, signatureInNBitcoinFormat)
                        .Compress()
                    |> NodeId
    ...
    static member TryCreate
        ...
        if (nodeId.Value <> nodeSecret.PubKey) then
            Error "Wrong Node secret"
        else
            let msgSigner =
                { new IMessageSigner with
                    member this.SignMessage data =
                        nodeSecret.SignCompact(data, false)
                }
    ...
    static member TryCreate
        ...
        result {
            ...
            | Some n when n <> nodeId -> return! Error("NodeId mismatch!")
            | _ ->
                let maybeExpiryDate =
                    tags.Fields
                    |> List.choose(
                        function
                        | ExpiryTaggedField x -> Some x
                        | _ -> None
                    )
                    |> List.tryExactlyOne
                ...
                | Some e when e < timestamp ->
                    return!
                        Error(
                            sprintf
                                "You can not newly create already expired invoice! timestamp: %A. expiryDate: %A"
                                (timestamp)
                                (e)
                        )
                | _ ->
                    ...
                    return
                        { r with
                            Signature = Some(signature, recvId)
                        }
        }

    override this.Equals o =
        match o with
        | :? PaymentRequest as other ->
            (this :> IEquatable<PaymentRequest>).Equals(other)
